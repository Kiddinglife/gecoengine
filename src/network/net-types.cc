...
unsigned int sockaddr2hashcode(const sockaddrunion* sa)
{
    ...
    unsigned int lastHash = SuperFastHashIncremental((const char*) &sa->sin.sin_port, sizeof(sa->sin.sin_port),
            local_saaf);
    if (local_saaf == AF_INET)
    {
        lastHash = SuperFastHashIncremental((const char*) &sa->sin.sin_addr.s_addr, sizeof(in_addr), lastHash);
    }
    else if (local_saaf == AF_INET6)
    {
        lastHash = SuperFastHashIncremental((const char*) &sa->sin6.sin6_addr.s6_addr, sizeof(in6_addr), lastHash);
    }
    else
    {
...
class BundleDataPos
{
    ...
    public:
        BundleDataPos(GecoNetPacket * pPacket, void * pCurr) :
                pPacket_(pPacket), pCurr_((uchar*) pCurr)
        {
            assert(pPacket->Body() <= pCurr && pCurr < pPacket->Back());
        }
        GecoNetPacket * pPacket() const
        {
            return pPacket_;
        }
        uchar * pData() const
...
const GecoNetInterfaceElement GecoNetInterfaceElement::REPLY("Reply", REPLY_MESSAGE_IDENTIFIER, VARIABLE_LENGTH_MESSAGE,
        4);
