...
public:
	...
	*/
	...
	{
		typedef eastl::intrusive_list<intrusive_stat_rate_of_change_t<TYPE>>::iterator cit;
...
private:
	...
	*/
	void addTotalWatcher(geco_watcher_director_t* pwatchers_, const char * name, intrusive_stat_rate_of_change_t< TYPE>& stat)
	{
		...
		GECO_WATCH(buf, stat, CAST_METHOD_R(TYPE, intrusive_stat_rate_of_change_t< TYPE>, total), "total val", pwatchers_);
	}
	...
	*/
	...
	{
		typedef double (intrusive_stat_rate_of_change_t< TYPE>::*FunctionPtr)() const;
		FunctionPtr fns[] =
		{
		...
		char buf[256];
		for (stat_entries_t::size_type i = 0; i < mstats_entries_.size(); ++i)
		{
			stat_entry_t & entry = mstats_entries_[i];
			geco_snprintf(buf, sizeof(buf), "%s/%sPerSecond", entry.first.c_str(), name);
			//add_watcher(buf, stat,fns[i],NULL,"avergae val", pwatchers_);
		}

		geco_watcher_base_t* ptr = new method_watcher_t< double, intrusive_stat_rate_of_change_t<TYPE> >(stat, &intrusive_stat_rate_of_change_t<TYPE>::getRateOfChange0,0,0);
		//pwatchers_->add_watcher(buf, *ptr, NULL);
		//ptr->set_comment("avergae val");
	}
	...
	*/
	...
	{
		initRateOfChange("averages/lastSample", 0.f);
