...
void selector::set_expected_event_on_fd(int sfd, int eventcb_type, int event_mask, cbunion_t action,
        void* userData)
{
    ...
    for (i = 0; i < socket_despts_size_; i++)
    {
        ...
        case EVENTCB_TYPE_USER:
            ...
            if (event_callbacks[i].action.user_cb_fun != NULL) event_callbacks[i].action.user_cb_fun(
                    socket_despts[i].fd, socket_despts[i].revents, &socket_despts[i].events,
                    event_callbacks[i].userData);
            break;

        case EVENTCB_TYPE_UDP:
            recvlen_ = nit_ptr_->recv_udp_packet(socket_despts[i].fd, internal_udp_buffer_,
            MAX_MTU_SIZE, &src, &src_addr_len_);
            saddr2str(&src, src_address, MAX_IPADDR_STR_LEN, &portnum_);
            //@checkme do we really need this? because we already have built-in handler which is recv_udp_packet()
...
//                    src_address, portnum_);
        ...
        case EVENTCB_TYPE_SCTP:
            recvlen_ = nit_ptr_->recv_ip_packet(socket_despts[i].fd, internal_dctp_buffer,
            ...
            if (event_callbacks[i].action.socket_cb_fun != NULL) event_callbacks[i].action.socket_cb_fun(
                    socket_despts[i].fd, &internal_dctp_buffer[iphdrlen], recvlen_, src_address,
                    portnum_);
...
void selector::add_stdin_cb(stdin_data_t::stdin_cb_func_t stdincb)
{
    ...
    set_expected_event_on_fd(STD_INPUT_FD, EVENTCB_TYPE_USER, POLLIN | POLLPRI, cbunion_,
            &stdin_input_data_);
    socket_despts_size_--;
...
int selector::poll(void (*lock)(void* data), void (*unlock)(void* data), void* data)
{
    ...
    return poll_fds(socket_despts, &socket_despts_size_, msecs, lock, unlock, data);
}
...
int selector::poll_fds(socket_despt_t* despts, int* count, int timeout, void (*lock)(void* data),
        void (*unlock)(void* data), void* data)
{
    ...
    else
    {
        ...
        if (ret > 0)
        {
            EVENTLOG1(VERBOSE, "############### event %d occurred, dispatch it#############",
                    (unsigned int )ret);
...
int network_interface_t::set_sockdespt_recvbuffer_size(int sfd, int new_size)
{
    ...
    EVENTLOG2(VERBOSE, "line 648 expected buffersize %d, actual buffersize %d", new_size,
            new_sizee);
    return new_sizee;
}
...
int network_interface_t::open_ipproto_geco_socket(int af, int* rwnd)
{
    ...
    {
        ERRLOG1(MAJOR_ERROR, "Try to bind sockdespt {%d} but failed !", sockdespt);
    }
    ...
    *rwnd = set_sockdespt_recvbuffer_size(sockdespt, *rwnd); // 655360 bytes
    if (*rwnd < 0)
    {
        ...
        ERRLOG1(MAJOR_ERROR, "setsockopt: Try to set SO_RCVBUF {%d} but failed !", *rwnd);
    }

    if (setsockopt(sockdespt, SOL_SOCKET, SO_REUSEADDR, (char*) &optval, opt_size) < 0)
    {
        safe_close_soket(sockdespt);
        ERRLOG(MAJOR_ERROR, "setsockopt: Try to set SO_REUSEADDR but failed ! {%d} ! ");
    }

    //setup mtu discover option
#if defined (__linux__)
    optval = IP_PMTUDISC_DO;
    if (setsockopt(sockdespt, IPPROTO_IP, IP_MTU_DISCOVER, (const char *) &optval, optval) < 0)
    {
        safe_close_soket(sockdespt);
        ERRLOG(MAJOR_ERROR, "setsockopt: Try to set IP_MTU_DISCOVER but failed ! ");
    }

    // test to make sure we set it correctly
    if (getsockopt(sockdespt, SOL_SOCKET, IP_MTU_DISCOVER, (char*) &optval, &opt_size) < 0)
    {
        safe_close_soket(sockdespt);
    ...
    {
        EVENTLOG1(INTERNAL_TRACE, "setsockopt: IP_PMTU_DISCOVER succeed!", optval);
    }
#endif
...
#else
        if (setsockopt(sockdespt, IPPROTO_IPV6, IPV6_PKTINFO, (const char*) &optval, optval) < 0)
        {
            safe_close_soket(sockdespt);
            return -1;
        }
    ...
    *rwnd = set_sockdespt_recvbuffer_size(sfd, *rwnd); // 650KB
...
#else
        ERRLOG2(MAJOR_ERROR, "setsockopt: Try to set SO_RCVBUF {%d} but failed {%d} !\n", *rwnd,
                errno);
#endif
    }
    ...
    if (setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (const char*) &ch, sizeof(ch)) < 0)
    {
        ...
        ERRLOG1(MAJOR_ERROR,
                ...
                WSAGetLastError());
#else
        ERRLOG1(MAJOR_ERROR, "setsockopt: Try to set SO_REUSEADDR but failed ! !\n", errno);
#endif
        return -1;
    }
...
int network_interface_t::send_udp_packet(int sfd, char* buf, int length, sockaddrunion* destsu)
{
    if (sfd <= 0)
    {
    ...
    case AF_INET6:
        ...
        result = sendto(sfd, buf, length, 0, (struct sockaddr *) &(destsu->sin6),
...
int network_interface_t::send_ip_packet(int sfd, char *buf, int len, sockaddrunion *dest, uchar tos)
{
    EVENTLOG(INFO, "- - - - - -Enter network_interface_t::send_ip_packet()- - - - - - ");
    ...
    case AF_INET:
        ...
        EVENTLOG6(VERBOSE, "AF_INET::send_ip_packet:set IP_TOS %u,"
                ...
                inet_ntoa(dest->sin.sin_addr), ntohs(dest->sin.sin_port));
    ...
    case AF_INET6:
        ...
        EVENTLOG4(VERBOSE, "AF_INET6::send_ip_packet:sfd:%d,len:%d,destination:%s::%u\n", sfd, len,
                hostname, ntohs(dest->sin6.sin6_port));
    ...
    EVENTLOG3(INFO, "Leave stat_send_event_size_ %u, stat_send_bytes_ %u, packet len %u\n",
            stat_send_event_size_, stat_send_bytes_, len - UDP_PACKET_FIXED_SIZE);

    return txmt_len;
}

int network_interface_t::recv_ip_packet(int sfd, char *dest, int maxlen, sockaddrunion *from,
        sockaddrunion *to)
{
...
int network_interface_t::recv_udp_packet(int sfd, char *dest, int maxlen, sockaddrunion *from,
        socklen_t *from_len)
{
    int len;
...
int network_interface_t::add_udpsock_ulpcb(const char* addr, ushort my_port, socket_cb_fun_t scb)
{
#ifdef _WIN32
    ERRLOG(MAJOR_ERROR,
    ...
    EVENTLOG1(VERBOSE, "Registered ULP-Callback: now %d registered callbacks !!!\n", new_sfd);
    return new_sfd;
}
...
bool network_interface_t::get_local_addresses(union sockaddrunion **addresses, int *numberOfNets,
        int sctp_fd, bool with_ipv6, int *max_mtu, const IPAddrType flags)
{
    ...
    *numberOfNets = 0;
    ...
    {
        ERRLOG(FALTAL_ERROR_EXIT, "Out of Memory in adl_gatherLocalAddresses() !");
        return (false);
    }
        ...
        while (fgets(addrBuffer, sizeof(addrBuffer), v6list) != NULL)
        {
            if (strncmp(addrBuffer, "00000000000000000000000000000001", 32) == 0)
            {
                ...
                if (inet_pton(AF_INET6, "::1", (void *) &sin6.sin6_addr) > 0)
                {
                    ...
                    EVENTLOG5(VERBOSE, "copied the local IPV6 address %x:%x:%x:%x, family %x",
                            sin6.sin6_addr.s6_addr32[3], sin6.sin6_addr.s6_addr32[2],
                            sin6.sin6_addr.s6_addr32[1], sin6.sin6_addr.s6_addr32[0],
                            sin6.sin6_family);
                    (*numberOfNets)++;
                }
                continue;
            ...
            {
                ERRLOG1(FALTAL_ERROR_EXIT, "Could not translate string %s", addrBuffer2);
            }
        }
        fclose(v6list);
    }
    ...
    for (ii = 0; ii < numAlocIPv4Addr; ii++, ifrequest = nextif)
    {
        ...
        *max_mtu = DEFAULT_MTU_CEILING;
        ...
        EVENTLOG3(VERBOSE, "Interface %d, NAME %s, Hex: %x", ii, local.ifr_name, local.ifr_name);
        ...
        {
            if (this->typeofaddr((union sockaddrunion*) toUse, LoopBackAddrType))
            {
                /* skip the loopback */
         ...
         * it
         */
        EVENTLOG2(VERBOSE, "Starting checking for duplicates ! MTU = %d, nets: %d", saveMTU,
                *numberOfNets);
        ...
        if (*numberOfNets)
        {
            ...
            for (xxx = 0; xxx < tmp; xxx++)
            {
                ...
                if (saddr_equals(&localAddresses[xxx], (union sockaddrunion*) toUse))
                {
#ifdef HAVE_IPV6
...
#endif
        (*numberOfNets)++;
        EVENTLOG2(VERBOSE, "Interface %d, Number of Nets: %d", ii, *numberOfNets);
    }

    EVENTLOG1(VERBOSE, "adl_gatherLocalAddresses: Found %d addresses", *numberOfNets);
    for (ii = 0; ii < (*numberOfNets); ii++)
    {
        saddr2str(&(localAddresses[ii]), addrBuffer2, MAX_IPADDR_STR_LEN, NULL);
        EVENTLOG2(VERBOSE, "adl_gatherAddresses : Address %d: %s", ii, addrBuffer2);
...
bool network_interface_t::typeofaddr(union sockaddrunion* newAddress, IPAddrType flags)
{
    switch (saddr_family(newAddress))
    {
    case AF_INET:
        EVENTLOG(VERBOSE, "Trying IPV4 address\n");
        if ((IN_MULTICAST(ntohl(newAddress->sin.sin_addr.s_addr)) && (flags & MulticastAddrType))
