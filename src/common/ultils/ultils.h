...
#define BYTES_TO_BITS(x) ((x)<<3)
		 ...
		 */
		GECOAPI bool GECO_FAST_CALL geco_utf8tow(const char * s, std::wstring& output);
...
#define geco_fopen ultils::win_fopen
		 ...
		 */
		GECOAPI bool GECO_FAST_CALL win_geco_acp2w(const char * src, std::wstring& output);
		GECOAPI bool GECO_FAST_CALL win_geco_acp2w(const std::string & s, std::wstring& output);
		GECOAPI std::wstring GECO_FAST_CALL  win_geco_acp2w(const std::string & s);
		GECOAPI FILE* GECO_FAST_CALL  win_fopen(const char* filename, const char* mode);
		GECOAPI long GECO_FAST_CALL win_geco_file_size(FILE* file);
		GECOAPI std::wstring GECO_FAST_CALL win_get_temp_file_path_name();
#else
#include<stdio.h>
