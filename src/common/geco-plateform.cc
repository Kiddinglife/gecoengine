...
#ifdef _WIN32
static const wchar_t *s_pcSvcName = NULL;
#else
static const char *s_pcSvcName = NULL;
#endif

extern int   g_iBackEndUID = 0;
extern int   g_iBackEndPID = 0;
extern int  g_iIsBackEndProcess = 0;

#ifndef _WIN32
#else 
#endif 

void  geco_open_syslog(
#ifndef _WIN32
	const char *pcIdent,
#else 
	const wchar_t *pcIdent,
#endif 
	int iOption, int iFacility)
{
#ifndef _WIN32
...
void  geco_syslog(int iPriority, const char *pcFormat, ...)
{
#ifndef _WIN32
	va_list ap;
	...
	HANDLE hHandle;
	va_list kArgList;
	static char s_acMsgBuf[1024];
	static std::wstring wstr;

	hHandle = RegisterEventSourceW(NULL,
		s_pcSvcName);
	if (hHandle == NULL)
		return;
	...
	va_end(kArgList);
	geco::ultils::win_geco_acp2w(s_acMsgBuf, wstr);

	PUTSTR(wstr.c_str());
	OutputDebugStringW(wstr.c_str());

	if (!ReportEventW(hHandle,
		iPriority,
		0,
		iPriority,
		NULL,
...
static INLINE void GECO_FAST_CALL memcpy_sse2_16(void *dst, const void *src)
{
	__m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
	_mm_storeu_si128(((__m128i *) dst) + 0, m0);
}
static INLINE void GECO_FAST_CALL memcpy_sse2_32(void *dst, const void *src)
{
	__m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
...
static INLINE void GECO_FAST_CALL memcpy_sse2_64(void *dst, const void *src)
{
	__m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
	__m128i m1 = _mm_loadu_si128(((const __m128i *) src) + 1);
...
static INLINE void GECO_FAST_CALL memcpy_sse2_128(void *dst, const void *src)
{
	__m128i m0 = _mm_loadu_si128(((const __m128i *) src) + 0);
	__m128i m1 = _mm_loadu_si128(((const __m128i *) src) + 1);
	__m128i m2 = _mm_loadu_si128(((const __m128i *) src) + 2);
...
static INLINE void*  GECO_FAST_CALL memcpy_tiny(void *dst, const void *src, size_t size)
{
...
void  geco_init_back_end_process(int iUID, int iPID)
{
	g_iIsBackEndProcess = 1;
	g_iBackEndPID = iPID;
	g_iBackEndUID = iUID;
}
int   geco_get_user_id()
{
#ifdef _WIN32
	// VS2005:
#if _MSC_VER >= 1400
...
LP_FV_PASSWD geco_get_pw_user_id(int iUID)
{
#ifdef _WIN32
...
LP_FV_PASSWD  geco_get_pw_nam(const char *pcName)
{
#ifdef _WIN32
	static struct passwd kPW = { "name","pwd",0,0,"","","" };
	return &kPW;
...
{
#ifdef _WIN32
	return s_pcSvcName ? s_pcSvcName : L"unknown";
#else
	// Note: a string in a static area is returned. Do not store this pointer.
	// See cuserid for details.
	char * pUsername = cuserid(NULL);
	return pUsername ? pUsername : (s_pcSvcName ? s_pcSvcName : "unknown");
#endif
}
const char*   get_base_path(const char * path, const char * module)
{
