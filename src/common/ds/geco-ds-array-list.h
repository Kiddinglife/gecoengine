...
public:
    ...
    array_list_t(const array_list_t& original_copy)
    {
        ...
        listArray = geco_new_array<ElementType>(original_copy.list_size,
                FILE_AND_LINE);
    ...
    void Shrink2MiniSzie(void)
    {
        ...
        ElementType * new_array = geco_new_array<ElementType>(mini_size,
                FILE_AND_LINE);
        // copy old array over
        for (unsigned int counter = 0; counter < list_size; ++counter)
            new_array[counter] = listArray[counter];
    ...
    void remove_slow(const unsigned int position)
    {
        ...
        if (position < list_size)
        {
            ...
            for (unsigned int counter = position; counter < list_size - 1;
                    ++counter)
                listArray[counter] = listArray[counter + 1];
