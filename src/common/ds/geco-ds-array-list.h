...
#include "../debugging/geco-malloc.h"
...
namespace geco_ds
{
    ...
    class GECO_EXPORT array_list_t
    {
        ...
        public:
        array_list_t() : allocation_size(0), listArray(0), list_size(0){}
        ~array_list_t()
        {
        ...
        array_list_t(const array_list_t& original_copy)
        {
            ...
            listArray = geco_new_array<ElementType>(original_copy.list_size, FILE_AND_LINE);
        ...
        void reserve(unsigned int countNeeded)
        {
            ...
            if (allocation_size < amount2Alloc)
            {
                ...
                new_array = geco_new_array<ElementType>(allocation_size, FILE_AND_LINE);
                if (listArray != 0)
                {
                    // copy old array over
        ...
        void Shrink2MiniSzie(void)
        {
            ...
            ElementType * new_array = geco_new_array<ElementType >(mini_size, FILE_AND_LINE);
            // copy old array over
            for (unsigned int counter = 0; counter < list_size; ++counter)
                new_array[counter] = listArray[counter];
        ...
        void push_back(const ElementType &input)
        {
            ...
            if (list_size == allocation_size)
            {
                ...
                ElementType * new_array =
                    geco_new_array<ElementType >(allocation_size, FILE_AND_LINE);
        ...
        void push(const ElementType &input, const unsigned int position)
        {
            ...
            if (list_size == allocation_size)
            {
                ...
                ElementType * new_array = geco_new_array<ElementType >(allocation_size, FILE_AND_LINE);
        ...
        void replace(const ElementType &input, const ElementType filler, const unsigned int position)
        {
        ...
        unsigned int capacity() const { return allocation_size; }
    };
}
