...
namespace GecoEngine {
    ...
    bool KBEBlowfish::init()
    {
        ...
        {
            //ERROR_MSG(fmt::format("KBEBlowfish::init: ""invalid length {}\n",keySize_));
        ...
        int length)
    {
        ...
        {
            //CRITICAL_MSG(fmt::format("Blowfish::encrypt: ""Input length ({}) is not a multiple of block size ({})\n", length, (int)(BLOCK_SIZE)));
        }
        ...
        int length)
    {
        if (length % BLOCK_SIZE != 0)
        {
            //ERROR_MSG(fmt::format("Blowfish::decrypt:" "Input stream size ({}) is not a multiple of the block size ({})\n",length, (int)(BLOCK_SIZE)));
                return -1;
        }
