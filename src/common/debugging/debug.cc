...
namespace geco
{
	namespace debugging
	{
		...
		{ "VERBOSE: ", "DEBUG: ", "INFO: ", "NOTICE: ", "WARN: ", "ERR: ", "CRIT: ",
				"HACK: ", "SCRIPT: ", "ASSET: " };
...
#include "../ultils/ultils.h"
		...
		{
			WCHAR acClassName[1024];
			GetClassName(hWnd, acClassName, sizeof(acClassName));
			if (lstrcmpW(acClassName, L"tooltips_class32") &&
				lstrcmpW(acClassName, L"#32770"))
			{
				DWORD iProcessID;
		...
		class win32_critical_msg_handler_t : public default_critical_msg_handler_t
		{
			...
			{
				std::wstring pcmsg;
				geco::ultils::win_geco_acp2w(msg, pcmsg);
				if (::MessageBox(GetDefaultParent(), pcmsg.c_str(), L"Critical Error", MB_YESNO) == IDYES)
...
#include <cxxabi.h>
		 ...
		 * 	This is a helper function used by the CRITICAL_MSG macro.
		 */
		void log_msg_helper::critical_msg_aux(bool isDevAssertion, const char * format,
			va_list argPtr)
		{
			...
			for (uint i = 0;
				i < log_msg_filter_t::get_instance().critical_msg_cbs_.size(); ++i)
			{
				va_list tmpArgPtr;
				geco_va_copy(tmpArgPtr, argPtr);
				(*(log_msg_filter_t::get_instance().critical_msg_cbs_[i]))(cpn_priority_, msg_priority_, format, tmpArgPtr);
				va_end(tmpArgPtr);
			}
			...
			geco_snprintf(filename, sizeof(filename), "assert.%s.%s.%d.log", pExeName,
				hostname, getpid());
		...
		void log_msg_helper::message(const char * format, ...)
		{
...
#endif
			if (LOG_MSG_CRITICAL == msg_priority_)
			{
				for (uint i = 0;
					i < log_msg_filter_t::get_instance().critical_msg_cbs_.size();
					++i)
				{
					if (!handled)
					{
						...
						handled =
							(*(log_msg_filter_t::get_instance().critical_msg_cbs_[i]))(
								cpn_priority_, msg_priority_, format, tmpArgPtr);
						va_end(tmpArgPtr);
					}
				}
			}
			...
			{
				for (uint i = 0;
					i < log_msg_filter_t::get_instance().debug_msg_cbs_.size(); ++i)
				{
					if (!handled)
					{
						va_list tmpArgPtr;
			...
			// simply to print it out if all cb return false
			if (!handled)
			{
				if (0 <= msg_priority_&&
