...
#else//defined(_WIN32)
default_critical_msg_handler_t* default_critical_msg_handler_t::handler_ = NULL;
		...
		va_list argPtr)
{
	...
	if (size > 0)
	{
		...
		{
			(*(log_msg_filter_t::get_instance().critical_msg_cbs_[i]))(buffer,
					(critical_msg_cb_tag*) 0);
		}
	}
