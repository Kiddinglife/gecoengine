...
template <class Type>
GECO_EXPORT Type* geco_new_array(const int count,
    const char *file, unsigned int line)
{
    if (count == 0)
        return 0;
...
GECO_EXPORT void geco_delete(Type *buff, const char *file, unsigned int line)
{
    if (buff == 0) return;
    buff->~Type();
    (geco_free_ext)((char*)buff, file, line);

}

template <class Type>
GECO_EXPORT void geco_delete_array(Type *buff, const char *file, unsigned int line)
{
    if (buff == 0)
        return;
    ...
    (geco_free_ext)((char*)buff - sizeof(int), sizeof(Type), file, line);
