...
static int check_randomized(const struct rand_cfg *cfg)
{
    ...
    wtimers_it it_p, it_e, it_all;
    int p_done = 0, e_done = 0, all_done = 0;
    wtimer_t *tos = NULL;
    const int rel = cfg->relative;

    geco::ultils::wtimers_t wtimers;
    wtimers.open(TIMEOUT_mHZ);

    EXPECT_EQ(!t || !timeouts || !tos || !fired || !found || !deleted, false);

    wtimers.update(cfg->start_at);
    for (i = 0; i < cfg->n_timeouts; ++i)
    {
        ...
        wtimers.add_timer(t + i, timeouts[i] - (rel ? now : 0));
    ...
    EXPECT_EQ(!!n_added_pending, wtimers.has_pending_timer());
    EXPECT_EQ(!!n_added_expired, wtimers.has_expired_timer());



}
TEST(GECO_ENGINE_ULTILS, TEST_WHEEL_TIMER_)
{
    /*1) check open() */
    geco::ultils::wtimers_t wtimers;
    wtimers.open(TIMEOUT_mHZ);
    EXPECT_EQ(wtimers.get_hz(), TIMEOUT_mHZ);
    EXPECT_EQ(wtimers.curtime, 0);
    for (int i = 0; i < WHEEL_NUM; i++)
    {
        EXPECT_EQ(wtimers.pending[i], 0);
    }

    /*2) check randomized timer*/
