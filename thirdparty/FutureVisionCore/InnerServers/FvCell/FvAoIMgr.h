...
public:
	...
	FvUInt32	QueryVision(FvAoIHandle hHandle, TVisiter* pkVisiter) const;

	template<typename TVisiter,
		void (TVisiter::*OnVisit)(FvAoIExt* pkExt)>
	FvUInt32	QueryVisionExt(FvAoIHandle hHandle, TVisiter* pkVisiter) const;

	template<typename TVisiter,
		void (TVisiter::*OnVisit)(void* pkObsData)>//! 去除Trap的被视野
	FvUInt32	QueryDVision(FvAoIHandle hHandle, TVisiter* pkVisiter) const;

	template<typename TVisiter,
		void (TVisiter::*OnVisit)(void* pkObjData)>//! 去除自己
	FvUInt32	QueryArea(FvAoIHandle hHandle, float fX, float fY, float fRadius, TVisiter* pkVisiter) const;

protected:
	FvInt32		TransPosAndScale(float fPos) const;
