...
class GridSubdivider
{
	...
	array_map<objectInfo> objects;
	SAP::BatchedSet<std::pair<ClientKey, ClientKey> > candidates;
...
public:
	...
	void GetSpatialBounds(std::vector<Coord>& mins, std::vector<Coord>& maxs)
	{
		mins.clear();
		maxs.clear();
		...
		{
			Coordinate min[D], max[D];
			spaces[spaceCoordinates[i].index]->GetSpatialBounds( min, max );

			for( int d = 0; d < D; ++d )
			{
				mins.push_back(min[d]);
				maxs.push_back(max[d]);
			}
		}
	}
