...
public:
		...
		if( insertion_sort_backward_minima( p ) && previousChunk != 0 )
		{
			...
			if( elements[1] < *remotePoint )
			{
				if( d == 0 )
				{
					...
					else//! Uman: 如果previous Chunk是空的怎么办?只有两个Sentinel
	...
	inline ThisType * next() { return nextChunk; }
...
public:
	...
	void overlap_work_for_dynamics( int aabbIndex, bool insertion )
	{
		...
		for( int i = 0; i < size; ++i ) if( keys[i] == aabbIndex ) { keys[i] = keys.back(); keys.pop_back(); break; } // Eliminate self collision on insert
...
public:
	...
	Segmented( Filter _filter ):
#pragma warning (push)
#pragma warning (disable: 4355)
	overlapManager( _filter, this ),
#pragma warning (pop)
	globalSort( false ),
	entityCount( 0 ),
	debugging( false ) {
	...
	int numBoxes() { return boxes.size(); }
	...
	void GetSpatialBounds( Coordinate boundsMin[], Coordinate boundsMax[] )
	{
		for( int d = 0; d < D; ++d )
		{
			boundsMin[d] = spatialBoundsMin[d];
			boundsMax[d] = spatialBoundsMax[d];
		}
	}
