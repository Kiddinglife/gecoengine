...
template<typename TAoIExt, typename TTrapExt>
FvInt32 FvAoIMgr<TAoIExt, TTrapExt>::TransPosAndScale(float fPos)
{
...
FvUInt32 FvAoIMgr<TAoIExt, TTrapExt>::QueryVision(FvAoIHandle hHandle, TVisiter* pkVisiter)
{
	FV_ASSERT(hHandle && hHandle->IsObserver());
	FvAoIObserver* pkObs = (FvAoIObserver*)hHandle;
...
FvUInt32 FvAoIMgr<TAoIExt, TTrapExt>::QueryDVision(FvAoIHandle hHandle, TVisiter* pkVisiter)
{
	FV_ASSERT(hHandle && hHandle->IsObject());
	FvAoIObject* pkObj = (FvAoIObject*)hHandle;
...
template<typename TVisiter,
void (TVisiter::*OnVisit)(void* pkObjData)>
FvUInt32 FvAoIMgr<TAoIExt, TTrapExt>::QueryArea(float fX, float fY, float fRadius, TVisiter* pkVisiter)
{
	FvInt32 iX = TransPosAndScale(fX);
	FvInt32 iY = TransPosAndScale(fY);
	FvInt32 iRadius = TransPosAndScale(fRadius);
	return m_kGrid.QueryArea<TVisiter, OnVisit>(iX, iY, iRadius, pkVisiter);
}
