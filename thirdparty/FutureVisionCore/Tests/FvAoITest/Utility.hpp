...
struct extrema
{
	...
	enum Which { Uninitialized = 0, Minima = 1, Maxima = 2, Sentinel = 3 };
...
public:
	...
	void sort()
	{
		...
		int from = (int)set.size() - 1;
		set.resize( set.size() + adds.size() );
		int to = (int)set.size() - 1;
		while( from >= 0 )
		{
		...
		from = 0, to = 0;
		int u = 0, size = (int)deletes.size(), size2 = (int)set.size();
		while( u < size )
		{
			if( deletes[u] == set[from] )//! Uman: 如果deletes里有一个set里没有的元素,则会导致该元素之后的所有元素都不会被删除
			{
				++from;
				++u;
			}
			...
			{
				//! Uman: 可以加判断,判断to和from是否相等,相等的时候不用赋值
				set[to++] = set[from++];
			}
		}
		size = (int)set.size();
...
public:
	...
	static float optimalEntitiesPerDomain() { return EventBasedAlgorithm::optimalEntitiesPerDomain(); }
	int numBoxes() { return itree.numBoxes(); }
	void SetSpatialBounds( Coordinate boundsMin[], Coordinate boundsMax[] ) { itree.SetSpatialBounds( boundsMin, boundsMax ); }
	bool SpaceFullyContainsAABB( Coordinate min[], Coordinate max[] ) { return itree.SpaceFullyContainsAABB( min, max ); }
	void clear() { itree.clear(); }
	int insert( ClientKey objectKey, Coordinate min[], Coordinate max[] ) { return itree.insert( objectKey, min, max ); }
	void erase( int key ) { itree.erase( key ); }
	void UpdateAABB( int key, Coordinate min[], Coordinate max[] ) { itree.UpdateAABB( key, min, max ); }
	void AcquireAABB( int key, Coordinate min[], Coordinate max[] ) { itree.AcquireAABB( key, min, max ); }
	void GetSpatialBounds(std::vector<Coordinate>& mins, std::vector<Coordinate>& maxs) { itree.GetSpatialBounds(mins, maxs); }
	void SetGridParameters( Coordinate boundsMin[], Coordinate boundsMax[], int _numCells[], int _numThreads = 1 ) { itree.SetGridParameters( boundsMin, boundsMax, _numCells, _numThreads ); }
...
public:
	...
	bool present( int key ) { return valid[key]; }
...
struct FilterNothing {
	bool pass( const KeyType& one, const KeyType& two ) { return true; }
};
