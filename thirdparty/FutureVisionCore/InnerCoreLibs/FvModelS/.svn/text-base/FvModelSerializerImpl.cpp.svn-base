...
FvXMLModelSerializerImpl::FvXMLModelSerializerImpl()
{
...
bool FvXMLModelSerializerImpl::ExportModel(const FvModel *pkModel, const FvString& kFileName)
{
	return true;
}

bool FvXMLModelSerializerImpl::ImportModel(Ogre::DataStreamPtr &kStream, FvModel *pkDest)
{
...
#ifndef FV_SERVER
void FvXMLModelSerializerImpl::ReadNode(FvXMLSectionPtr spSection,
	SceneNode *pkParent,FvModel *pkDest)
{
...
void FvXMLModelSerializerImpl::ReadEntity(FvXMLSectionPtr spSection,
				Ogre::SceneNode *pkNode,FvModel *pkDest)
{
	...
	if(pkAnimations)
	{
		...
		while (kIt.hasMoreElements())
		{
			...
			if(pkDest->GetAnimation(pkAnim->getAnimationName()) == NULL)
				...
				FvModel::ModelAnimations::value_type(pkAnim->getAnimationName(),pkAnim));
		}
	}*/
...
void FvXMLModelSerializerImpl::ReadAnimation(FvXMLSectionPtr spSection,
				   Ogre::SceneNode *pkNode,FvModel *pkDest)
{
	Ogre::SceneManager *pkSceneManager = Ogre::Root::getSingleton().
		_getCurrentSceneManager();
	...
	if(!pkSceneManager->hasAnimation(kAnimationName))
	{
		std::vector<FvXMLSectionPtr> kKeyFrames;
		spSection->OpenSections("keyframe",kKeyFrames);

		Animation *pkAnimation = pkSceneManager->createAnimation(kAnimationName, fLength);
		...
		AnimationState *pkAnimationState = pkSceneManager->createAnimationState(kAnimationName);
		pkAnimationState->setEnabled(bEnable);
		pkAnimationState->setLoop(bLoop);
		pkDest->m_kModelAnimations.insert(
			FvModel::ModelAnimations::value_type(kAnimationName,pkAnimationState));
	}
}
#endif // !FV_SERVER

void FvXMLModelSerializerImpl::ReadCollision(FvXMLSectionPtr spSection,FvModel *pkDest)
{
